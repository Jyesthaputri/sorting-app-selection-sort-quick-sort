...
def quick_sort_steps(arr):
    ...
    while stack:
        # Ambil batas subarray yang akan diproses dari stack
        low, high = stack.pop()
        ...
        # Tampilkan elemen pertama (a[low]) sebagai kandidat pivot
        yield a[:], low, high, None, None, (
            f"Pivot dipilih: a[{low}]={a[low]} (elemen pertama)"
        )

        # Tukar a[low] ke posisi a[high] agar logika partisi selalu menggunakan a[high] sebagai pivot tanpa perlu diubah
        a[low], a[high] = a[high], a[low]
        yield a[:], low, high, None, (low, high), (
            f"Tukar a[{low}] & a[{high}] → pindah pivot ke index {high}"
        )
        # Setelah ditukar, pivot sekarang ada di posisi paling kanan (di indeks tertinggi)
        pivot = a[high]
        i = low - 1 #i merupakan pointer batas elemen lebih besar dari pivot, disini blm ada elemen di proses (i = -1)

        yield a[:], low, high, high, None, f"Pivot dipilih (Pertama): {pivot} (index {high})"
        ...
        for j in range(low, high):      #j: pointer yang berjalan dari low sampai sebelum high
            ...
            # Descending: a[j] lebih besar dari pivot,
            # pindahkan ke sisi kiri (posisi setelah i)
            if a[j] > pivot:
                i += 1
                a[i], a[j] = a[j], a[i]
...
class QuickSortFirstVisualizerPage(QWidget):
    def __init__(self):
        super().__init__()
        ...
        # ── Judul ─────────────────────────────────────────────────────────────
        self.title = QLabel("VISUALISASI QUICK SORT (PIVOT INDEKS KE-0)")
